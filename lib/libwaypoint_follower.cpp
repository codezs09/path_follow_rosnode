...
geometry_msgs::Point calcRelativeCoordinate(geometry_msgs::Point point_msg, geometry_msgs::Pose current_pose)
{
  ...
  //ROS_INFO_STREAM("current pose in calcRelativeCoord " << current_pose);
  tf::Point p;
...
int getClosestWaypoint(const styx_msgs::Lane &current_path, geometry_msgs::Pose current_pose)
{
  ...
  for (int i = 1; i < wp.getSize(); i++)
  {
    if (getPlaneDistance(wp.getWaypointPosition(i), current_pose.position) > search_distance)
       {double temp_distance = getPlaneDistance(wp.getWaypointPosition(i), current_pose.position);
        //ROS_INFO_STREAM("distance" << temp_distance);
        continue;}

    if (!wp.isFront(i, current_pose))
    { ROS_INFO_STREAM("wp is not front");
      continue;
    }
    double angle_threshold = 90;
    //ROS_INFO_STREAM("relative angle" <<getRelativeAngle(wp.getWaypointPose(i), current_pose));
  ...
  else
  {
    ...
    ROS_INFO_STREAM("waypoint_min" << waypoint_min);
    return waypoint_min;
